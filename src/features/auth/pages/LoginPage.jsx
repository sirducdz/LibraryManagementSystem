...
import { LockOutlined, UserOutlined, KeyOutlined, WarningOutlined } from "@ant-design/icons";
import { Button, Checkbox, Form, Input, Card, Typography, Alert, Divider } from "antd";
// import { Link } from "react-router-dom"; // Import nếu cần
import { useLoginMutation } from "../Store/Service/authApi"; // Điều chỉnh đường dẫn nếu cần

const { Title } = Typography;

const LoginPage = () => {
  const [loginTrigger, { isLoading, error, data, isSuccess, isError, reset }] = useLoginMutation();
  ...
  const getErrorMessage = (error) => {
    ...
    if (error.data) {
      if (typeof error.data === 'object' && error.data !== null && error.data.message) {
        return error.data.message;
      }
      if (typeof error.data === 'object' && error.data !== null && error.data.error) {
          ...
          <div className="p-4">
            {isError && (
              <Alert
                ...
                message={<span className="font-semibold text-black">Login Failed</span>} // Đổi thành text-black
                description={<span className="text-black">{getErrorMessage(error)}</span>} // Đổi thành text-black
                type="error"
                showIcon
                closable
                onClose={reset}
            ...
            >
              ...
              <Form.Item
                name="username"
                label={<span className="font-semibold text-black text-sm">Username / Email</span>}
              ...
              <Form.Item
                name="password"
                label={<span className="font-semibold text-black text-sm">Password</span>}
              ...
              <Form.Item className="mb-3">
                <div className="flex justify-between items-center">
                  <Form.Item name="remember" valuePropName="checked" noStyle>
                    <Checkbox className="text-white text-sm font-medium">Remember me</Checkbox>
                  </Form.Item>
                  <a
              ...
              <div className="flex gap-3 justify-center mt-3">
                 ...
                 <Button
                   ...
                   icon={ <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/><path d="M1 1h22v22H1z" fill="none"/></svg> }
                 />
                  {/* Facebook */}
                 <Button
                   className="flex items-center justify-center w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 border-0 text-white transition-all duration-300 hover:-translate-y-1"
                   icon={ <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.84 3.44 8.87 8 9.8V15H8v-3h2V9.5C10 7.57 11.57 6 13.5 6H16v3h-2c-.55 0-1 .45-1 1v2h3v3h-3v6.95c5.05-.5 9-4.76 9-9.95z" /></svg> }
                 />
                  {/* GitHub */}
                 <Button
                   className="flex items-center justify-center w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 border-0 text-white transition-all duration-300 hover:-translate-y-1"
                   icon={ <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385c.6.105.825-.255.825-.57c0-.285-.015-1.23-.015-2.235c-3.015.555-3.795-.735-4.035-1.41c-.135-.345-.72-1.41-1.23-1.695c-.42-.225-1.02-.78-.015-.795c.945-.015 1.62.87 1.845 1.23c1.08 1.815 2.805 1.305 3.495.99c.105-.78.42-1.305.765-1.605c-2.67-.3-5.46-1.335-5.46-5.925c0-1.305.465-2.385 1.23-3.225c-.12-.3-.54-1.53.12-3.18c0 0 1.005-.315 3.3 1.23c.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23c.66 1.65.24 2.88.12 3.18c.765.84 1.23 1.905 1.23 3.225c0 4.605-2.805 5.625-5.475 5.925c.435.375.81 1.095.81 2.22c0 1.605-.015 2.895-.015 3.3c0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z" /></svg> }
                 />
              </div>
